...
export function ProductCard(product, index = 0) {
  ...
  // Generate responsive image sources - PNG for specific products, JPG for others
  const basePath = `/audiophile/assets/product-${product.slug}`;
  
  // Check if this product was converted to PNG
  const pngProducts = ['xx99-mark-one-headphones', 'yx1-earphones', 'zx9-speaker'];
  const imageExtension = pngProducts.includes(product.slug) ? 'png' : 'jpg';
  
  const desktopImage = `${basePath}/desktop/image-category-page-preview.${imageExtension}`;
  const tabletImage = `${basePath}/tablet/image-category-page-preview.${imageExtension}`;
  const mobileImage = `${basePath}/mobile/image-category-page-preview.${imageExtension}`;
